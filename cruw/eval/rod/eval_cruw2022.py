...
def evaluate_rodnet_cruw2022(res_dets, gt_dets, n_frame, dataset):

    olss_all = {(imgId, catId): compute_ols_dts_gts(gt_dets, res_dets, imgId, catId, dataset) \
                for imgId in range(n_frame)
                for catId in range(3)}

    evalImgs = [evaluate_img(gt_dets, res_dets, imgId, catId, olss_all, olsThrs, recThrs, dataset)
                for imgId in range(n_frame)
                for catId in range(3)]
